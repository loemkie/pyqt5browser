...
ping_host = encryutil.decrypt_des(p_token)
...
class OMClient(threading.Thread):
    def ping(self):
        try:
            self.pingClient = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.pingClient.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)  # 在客户端开启心跳维护
            self.pingClient.connect((ping_host, int(p_port)))
            return True
        except:
            return False
        return False
    ...
    def run(self):
        ...
        while True:
            ...
            #检测A库是否通，如果不通就关闭浏览器
            # if isBk == False:
            #     log.info("######检测AIS2.0心跳开始######")
            #     if not self.ping():
            #         log.info("检测AIS2.0心跳失败,强行关闭客户端")
            #         #关闭浏览器
            #         os._exit(5)
        ...
        if result["type"] == 4.0:#拨号
            ...
            try:
                if "id" in result.keys():
                    ...
                    omClient.send(result["outerTo"],result["extId"],result["omServer"]);
                else:
...
def settting(object_settings):
    ...
    glob_settings.setAttribute(13,True)
...
def downloadRequested(download):
    ...
    try:
        ...
        filename, ok2 = QFileDialog.getSaveFileName(dialog, "下载",download.path(), 'File (*'+ext+')');
...
def hostInit():
    hosts = [
                # '10.100.81.215  ais-cloud-gateway.fore.run',
...
if __name__ == "__main__":
    ...
    #控制主机#录音服务器omshost 9999端口是否通 start
    ...
    if version>="3.0":
        url = url_app;
    #end
    # url ="http://10.100.81.187:8186/oms-file-server/doc/2019/07/19/20190719160820080.pdf"
